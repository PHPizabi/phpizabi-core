...
///////////////////////////////////////////////////////////////////////////////////////
	...
	*/
	$select = myQ("
		SELECT SQL_CALC_FOUND_ROWS *, MATCH (`profile_data`) AGAINST ('".me("profile_data")."' IN BOOLEAN MODE) AS `score`
		FROM `[x]users` 
		WHERE MATCH (`profile_data`) AGAINST ('".me("profile_data")."' IN BOOLEAN MODE) 
		ORDER BY `score` DESC
		LIMIT 50
	");
	...
	*/
	...
	while ($row = myF($select)) {
		...
		if ($i==0) $fullMatchPoints = $row["score"];
		
		$resultsLoopArray[$i]["user.matchScore"] = round($row["score"] / $fullMatchPoints * 100, 1);
