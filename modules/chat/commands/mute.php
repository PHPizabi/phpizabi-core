			...
			if (isset($param[1])) {
				...
				if (strstr($usersList, "O:".$this -> nickname.",") or isset($_SESSION["triton"]["ircop"])) {
					...
					if (preg_match('/[A-Z]:'.preg_quote($param[1]).',/i', $usersList)) {
						...
						/* User is not already muted? */
						if (!preg_match('/[MO]:'.preg_quote($param[1]).',/i', $usersList)) {
						
							/* M'kay! Let's do it */
							$usersList = preg_replace('/[A-Z]:('.preg_quote($param[1]).'),/i', 'M:\\1,', $usersList);
							$this -> setChannelData($channel, "users", $usersList);
							
							$this -> sysMsg('SYS_MUTE', array($param[1], $this -> nickname));
						}
					}
