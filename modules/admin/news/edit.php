...
///////////////////////////////////////////////////////////////////////////////////////
	...
	foreach ($newsArray as $key => $newsEntity) {
		if ($newsEntity["date"] == $_GET["id"]) {
			$news = $newsEntity;
			$newsKey = $key;
			break;
		}
	...
	$tpl -> AssignArray(array(
		...
		"news.body" => $news["body"],
		"ck.lvl.guests" => (in_array("g", $news["access"]) ? "checked=\"checked\"" : NULL),
		"ck.lvl.users" => (in_array("u", $news["access"]) ? "checked=\"checked\"" : NULL)
	));
	...
	foreach ($heritage as $key => $levelArray) {
		...
		$levelsReplacementArray[] = array(
			...
			"level.checked" => (in_array($key, $news["access"]) ? "checked=\"checked\"" : NULL)
		);
	}
