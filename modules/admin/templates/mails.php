...
///////////////////////////////////////////////////////////////////////////////////////
	...
	// GET A LIST OF ALL THE MAILS TEMPLATES ////////////////////////////////////
	$totalMails = 0;
	
	if ($handle  = opendir("theme/templates/GLOBALS/mails/")) {
		while (false !== ($file = readdir($handle))) {
			if ($file != "." && $file != ".." && substr($file, strlen($file)-4) == ".tpl") {
				$tplList[] = array(
					"mail.file" => substr($file, 0, strlen($file)-4),
					"mail.size" => round(filesize("theme/templates/GLOBALS/mails/{$file}") / 1024, 2)
				);
				$totalMails ++;
			}
		}
	}
	$tpl -> Loop("mailsList", $tplList);
	$tpl -> AssignArray(array("mailsCount.total" => $totalMails));
