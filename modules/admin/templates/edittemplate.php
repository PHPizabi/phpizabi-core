...
///////////////////////////////////////////////////////////////////////////////////////
	...
	if (isset($_POST["Submit"])) {
		if ($handle = fopen($_GET["id"], "w")) {
			fwrite($handle, strtr(stripslashes($_POST["body"][0]), array_flip($entities)));
			fclose($handle);
			_fnc("reload", 0, "?L=admin.templates.templates");
		}
	}
	...
	$tpl -> AssignArray(array(
		"template.pageContent" => strtr(@file_get_contents($_GET["id"]), $entities),
		"template.pageName" => $_GET["id"]
	));
	
	if (!is_writable($_GET["id"])) {
